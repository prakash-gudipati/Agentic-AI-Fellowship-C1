...
def _generate_with_ragas(docs: List[Doc], *, n_rows: int) -> List[GoldenRow]:
    ...
    try:
        ...
        testset = gen.generate_with_langchain_docs(
            documents=lc_docs, test_size=n_rows)
    except ImportError:
        # langchain_core not installed — fall back to the direct text API
        # available on some Ragas versions.
        gen = TestsetGenerator()  # type: ignore[call-arg]
        testset = gen.generate(documents=[d.text for d in docs],
                               test_size=n_rows)  # type: ignore[call-arg]
