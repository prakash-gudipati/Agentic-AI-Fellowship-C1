...
def _evaluate_with_ragas(run_outputs: List[Dict[str, Any]], *,
    ...
    Ragas 0.2.x changed the auto-instantiation behaviour for embeddings
    and LLMs — passing a plain `OpenAIEmbeddings()` triggers an
    AttributeError on `embed_query`. The production pattern is to wrap
    LangChain components in Ragas's adapter wrappers (LangchainLLMWrapper,
    LangchainEmbeddingsWrapper) and pass them explicitly to evaluate().
    That also kills the "1 generation instead of 3" warning.
    """
    ...
    # Explicit judge + embedder wrapping — the fix for the 0.2.x AttributeError.
    # max_tokens=4096 because Ragas's faithfulness metric extracts atomic
    # claims and reasons about each — a 512-cap raises
    # `LLMDidNotFinishException` mid-claim. 4096 is well inside Haiku 4.5's
    # window and gives the judge enough room for the longest answers in
    # the golden dataset.
    judge_llm = LangchainLLMWrapper(ChatAnthropic(
        model=judge_model, max_tokens=4096, temperature=0.0))
    judge_embeddings = LangchainEmbeddingsWrapper(
        OpenAIEmbeddings(model="text-embedding-3-small"))

    # Throttle Ragas's parallel judge calls. Anthropic's default tier caps
    # Haiku at 50 requests/minute; Ragas defaults to max_workers=16 which
    # easily blows past 50 RPM when running 5 metrics × N rows. max_workers=4
    # + timeout=120s + 6 internal retries on 429 keeps us under the limit and
    # auto-recovers from transient rate-limit bursts.
    run_config = RunConfig(max_workers=4, timeout=120, max_retries=6)

    print("[ragas] calling ragas.evaluate() — issuing LLM-as-judge calls "
          "(throttled to 4 concurrent workers to respect 50 RPM limits)...")
